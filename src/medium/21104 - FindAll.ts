/*
    ...
    Given a pattern string P and a text string T, implement the type `FindAll<T, P>` that returns an
    Array that contains all indices (0-indexed) from T where P matches.
    ...
    : T extends ""
        ...
        : T extends `${P}${string}`
            ? T extends `${infer L}${infer R}`
                ? L extends ""
                    ? never
                    : [I["length"], ...FindAll<R, P, [...I, unknown]>]
                : never
            : T extends `${string}${infer R}`
                ? FindAll<R, P, [...I, unknown]>
                : never;
