...
type Contains<T extends unknown[], U> = T extends [infer L, ...infer R]
    ? (<A>() => A extends L ? 1 : 2) extends (<A>() => A extends U ? 1 : 2)
        ? true
        : Contains<R, U>
    : false;

type Unique<T extends unknown[], U extends unknown[] = []> = T extends [infer L, ...infer R]
    ? Unique<R, Contains<U, L> extends true ? U : [...U, L]>
    : U;
