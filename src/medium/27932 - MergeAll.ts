...
type MergeAll<XS extends object[], Res = object> = XS extends [infer L, ...infer R extends object[]]
    ? MergeAll<R, {
        [K in keyof L]: K extends keyof Res ? L[K] | Res[K] : L[K];
    } & {
        [K in keyof Res as K extends keyof L ? never : K]: Res[K];
    }>
    : {
        [K in keyof Res extends never ? never : keyof Res]: Res[K];
    };
