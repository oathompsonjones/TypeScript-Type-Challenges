/*
    ...
    that is, it switches the row and column indices of the matrix A by producing another matrix, often denoted by A<sup>T</sup>.
...
type Transpose<M extends number[][], R = M["length"] extends 0 ? [] : M[0]> = {
    [I in keyof R]: {
        [J in keyof M]: I extends keyof M[J]
            ? M[J][I]
            : never;
    }
};
