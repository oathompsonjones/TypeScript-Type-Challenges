/*
    ...
    `A` as the second, and will produce function type `G` which will be the same as `Fn` but with appended
...
type AppendArgument<Fn extends (...args: never[]) => unknown, A> = Fn extends (...args: infer T) => infer U
    ? (...args: [...T, A]) => U
    : never;
