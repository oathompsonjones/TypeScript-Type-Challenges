/*
    ...
    Get all possible paths that could be called by [_.get](https://lodash.com/docs/4.17.15#get)
    (a lodash function) to get the value of an object
    ...
    type T3 = ObjectKeyPaths<{ books: [{ name: string; price: number }] }>; // expected to be the
    superset of 'books' | 'books.0' | 'books[0]' | 'books.[0]' | 'books.0.name' | 'books.0.price' | 'books.length' | 'books.find'
    ```
...
type ObjectKeyPaths<T, K extends keyof T = keyof T> = T extends object
    ? K extends keyof T & (number | string)
        ? `${T extends unknown[] ? K | `[${K}]` : K}${"" | `${T[K] extends unknown[] ? "." | "" : "."}${ObjectKeyPaths<T[K]>}`}`
        : never
    : never;
