...
type BuildTuple<T extends number, R extends unknown[] = []> = R["length"] extends T
    ? R
    : BuildTuple<T, [unknown, ...R]>;
type Add<T1 extends number, T2 extends number> = T1 extends number
    ? T2 extends number
        ? [...BuildTuple<T1>, ...BuildTuple<T2>]["length"]
        : never
    : never;
type TwoSum<T extends number[], U extends number> = T extends [infer F extends number, ...infer R extends number[]]
    ? U extends Add<F, R[number]>
        ? true
        : TwoSum<R, U>
    : false;
