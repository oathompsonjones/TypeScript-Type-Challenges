/*
    ...
    As we know, union is an unordered structure, but tuple is an ordered, which implies that we are
    not supposed to preassume any order will be preserved between terms of one union, when unions are created or transformed.
    ...
    And a union could collapes, which means some types could absorb (or be absorbed by) others and
    there is no way to prevent this absorption. See the following examples:
    ```ts
...
type UnionToTuple<T> = [T] extends [never]
    ? []
    : UnionToIntersection<T extends unknown ? (t: T) => T : never> extends (_: infer R) => void
...
type ExtractValuesOfTuple<T extends unknown[]> = T[number & keyof T];
...
export type Cases = [
    ...
    Expect<Equal<ExtractValuesOfTuple<UnionToTuple<"a" | "b" | "c" | "d" | "e" | "f" | "g" | 1 | 2>>,
    "a" | "b" | "c" | "d" | "e" | "f" | "g" | 1 | 2>>
];
