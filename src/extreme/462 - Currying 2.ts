...
type Curried<Args extends unknown[], Return, T extends unknown[] = []> = Args extends [unknown, ...infer Rest]
    ? Args extends [...infer First, ...Rest]
        ? Rest extends []
            ? (...args: [...T, ...First]) => Return
            : Curried<Rest, Return, [...T, ...First]> & ((...args: [...T, ...First]) => Curried<Rest, Return>)
        : Return
    : never;
declare function DynamicParamsCurrying<A extends unknown[], R>(fn: (...args: A) => R): Curried<A, R>;
...
const curried2 = DynamicParamsCurrying((
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    _a: string, _b: number, _c: boolean, _d: boolean, _e: boolean, _f: string, _g: boolean
